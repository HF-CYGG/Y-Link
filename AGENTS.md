...
- **表单输入宽度**：后台配置类表单中的 `ElInput`、`ElSelect`、`ElInputNumber` 等输入控件，默认参考“系统配置 / 订单流水”模式：控件在所属表单项或卡片列容器内使用 `w-full` / `width: 100%` 铺满当前可用宽度，同一组字段保持等长；不要随意固定为 `240px`，也不要在宽屏下无限拉伸到跨越不相关区域。仅在短筛选项、工具栏、紧凑搜索框等明确需要固定尺寸的场景，才可按 Element Plus 示例设置具体宽度。
