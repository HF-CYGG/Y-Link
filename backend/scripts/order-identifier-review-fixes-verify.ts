...
try {
  ...
  assert.match(mysqlMigrationRunnerSource, /export async function applyMysqlOrderIdentifierNamespacesInTransaction\([\s\S]*055_order_identifier_namespaces\.sql[\s\S]*splitSqlStatements/, 'MySQL 共用领养入口必须执行 055 真源')
  assert.match(mysqlMigrationRunnerSource, /startTransaction\(\)[\s\S]*applyMysqlOrderIdentifierNamespacesInTransaction\(queryRunner\.manager\)[\s\S]*commitTransaction\(\)/, 'MySQL 启动领养必须在事务内原子执行 055')
