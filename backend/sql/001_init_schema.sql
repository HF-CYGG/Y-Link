...
CREATE TABLE IF NOT EXISTS `biz_inbound_order` (
  ...
  `cancel_reason` VARCHAR(255) NULL COMMENT '供货方撤销原因',
  `cancelled_at` DATETIME NULL COMMENT '撤销时间',
  `cancelled_by_user_id` BIGINT UNSIGNED NULL COMMENT '撤销操作人ID',
  `cancelled_by_username` VARCHAR(64) NULL COMMENT '撤销操作人账号快照',
  `cancelled_by_display_name` VARCHAR(128) NULL COMMENT '撤销操作人姓名快照',
