/**
 * 模块说明：backend/src/entities/biz-outbound-order.entity.ts
 * 文件职责：出库单主表实体，定义单号、状态、金额、幂等键与业务附加字段。
 * 实现逻辑：主单字段为出库服务、O2O 联动与列表查询提供统一持久化模型。
 * 维护说明：涉及金额、状态或幂等键字段变更需同步校验约束与回归脚本断言。
 */
