...
export class O2oPreorderItem {
  ...
  @Column({ name: 'discount_rate', type: 'decimal', precision: 3, scale: 1, default: 10.0, comment: '下单折扣快照' })
  discountRate!: string

  @Column({ name: 'unit_price', type: 'decimal', precision: 12, scale: 2, default: 0, comment: '下单折后单价快照' })
  unitPrice!: string

  @Column({ name: 'line_amount', type: 'decimal', precision: 14, scale: 2, default: 0, comment: '下单行金额快照' })
  lineAmount!: string
