/**
 * 模块说明：backend/src/services/auth.service.ts
 * 文件职责：管理端认证服务，负责登录校验、会话创建/注销、密码更新与当前用户解析。
 * 实现逻辑：
 * - 登录时完成账号状态校验、密码验证与会话落库；
 * - 会话解析支持 Cookie 与 Bearer 兼容；
 * - 密码策略与哈希升级逻辑在此处统一收口。
 * 维护说明：
 * - 认证返回结构或会话策略变更时，需同步调整 auth routes 与前端 auth store。
 */
