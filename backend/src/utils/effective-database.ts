/**
 * 文件说明：数据库生效状态摘要工具，统一生成“当前实际生效数据库”与“运行时覆盖文件状态”的可读说明。
 * 实现逻辑：同时对比进程启动时已加载的环境配置与磁盘上的当前覆盖文件，输出供健康检查、迁移治理页和启动日志复用的中文摘要。
 * 维护重点：若调整数据库配置加载优先级或覆盖文件结构，需要同步校验这里的状态判断与展示文案口径。
 */
