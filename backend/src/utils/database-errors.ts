/**
 * 模块说明：backend/src/utils/database-errors.ts
 * 文件职责：数据库异常识别工具，统一识别唯一键冲突、锁冲突与驱动错误类型。
 * 实现逻辑：
 * - 对 SQLite/MySQL 常见错误码做归类判断；
 * - 为 service 层重试与业务错误映射提供基础判断函数。
 * 维护说明：
 * - 引入新数据库方言或驱动升级后需补充错误码映射。
 */
