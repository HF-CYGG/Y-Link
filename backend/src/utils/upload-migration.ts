/**
 * 模块说明：backend/src/utils/upload-migration.ts
 * 文件职责：在后端启动时自动迁移历史上传文件与数据库中的旧附件 URL，避免上传目录分层后旧图片失效。
 * 实现逻辑：
 * - 仅迁移历史单层路径 `/uploads/<file>`，不会改动已经升级成分类目录的记录；
 * - 先移动物理文件到新目录，再更新数据库 URL，确保前端显示路径与磁盘位置一致；
 * - 迁移过程保持幂等，重复启动只会跳过已完成项，不会重复搬运或重复改写。
 * 维护说明：
 * - 当前迁移覆盖商品缩略图与反馈消息附件两类历史图片；
 * - 若后续新增新的上传业务模块，请同步把旧 URL 迁移规则加到本文件。
 */
