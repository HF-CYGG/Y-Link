/**
 * 文件说明：上传存储工具，统一维护图片上传的分类落盘、白名单校验、服务端重编码和静态访问 URL 生成规则。
 * 实现逻辑：所有图片按业务分类写入 `uploads/<category>` 子目录，并在上传前集中完成类型、体积和输出格式控制。
 * 维护重点：新增上传分类或调整尺寸参数时，需要同步核对旧路径兼容、前端访问方式和异常图片拦截策略。
 */
