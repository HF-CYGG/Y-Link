 ...
 * 实现逻辑：把传入的异步处理器包装成标准 RequestHandler，保留请求 operation identity 到完整 Promise 结束，并统一调用 next 捕获异常。
...
export function asyncHandler(
  ...
  return (req, res, next) => {
    void databaseOperationGate
      .retainCurrentOperation(() => handler(req, res, next))
      .catch(next)
  }
}
