/**
 * 模块说明：backend/src/constants/database-migration-copy.ts
 * 文件职责：统一收口后端服务、运行时状态摘要与自动验证脚本共用的数据库迁移提示文案，避免同一动作在不同模块出现不同说法。
 * 实现逻辑：
 * 1. 将“数据库迁移助手”设为数据库治理功能标准名称，供服务、脚本和前端页面对齐；
 * 2. 将“写入运行时覆盖后，需要重启后端服务才会生效”设为核心风险提示，复用到切换、回退和状态提醒；
 * 3. 将推荐闭环与默认原因抽成常量，降低后续服务逻辑、审计文案与验证脚本同步维护成本。
 * 维护说明：迁移流程或默认审计原因变更时，需同步核对本文件导出的阶段文本、切换/回退原因与前端常量口径。
 */
