 ...
 * 文件职责：封装“路由状态与工作台标签页绑定”的共享组合式逻辑，供产品中心、用户中心等多标签工作台复用。
 * 维护说明：
 * - 调用方只需要提供路由名、标签值、目标路径和承载组件之间的映射关系，本文件统一收口当前标签推导、当前组件选择和标签切换跳转；
 * - 若后续新增同类工作台页面，优先复用本组合式函数，而不是在页面内重复拼装 `useRoute`、`useRouter` 与多段 `computed`。
 */
