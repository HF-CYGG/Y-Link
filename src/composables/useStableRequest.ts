 ...
 * 文件职责：提供“仅保留最后一次请求结果”的稳定请求编排能力，统一治理列表筛选、切页与详情刷新时的旧请求回写问题。
 * 实现逻辑：
 * - 每次发起新请求前自动取消上一轮同类请求，避免慢请求晚返回时覆盖最新界面状态；
 * - 对成功、失败、取消和收尾阶段分别提供回调，让页面层按需编排加载态与提示逻辑；
 * - 在组件卸载或缓存页停用时主动中止未完成请求，减少无效网络占用与异步状态污染。
 */
