 ...
 * 文件职责：负责客户排行项的钻取抽屉，展示指定客户在统计区间内的订单与金额明细。
 * 实现逻辑：
 * - 抽屉按需加载客户钻取数据，避免仪表盘首页同步背上全部客户明细请求；
 * - 组件通过监听打开参数切换查询目标，并用稳定请求能力兜底快速切换时的结果覆盖问题。
 * 维护说明：
 * - 若后续扩展客户标签、部门维度或成交趋势，优先继续收口在抽屉中，不要抬高排行卡片复杂度；
 * - 客户名称、统计时间和金额字段展示必须和排行榜口径一致，避免前后解释不一致。
 */
