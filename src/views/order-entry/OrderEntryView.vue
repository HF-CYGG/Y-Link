 ...
 * 文件职责：作为管理端开单页壳层，负责装配头部信息卡、商品明细编辑区和底部汇总提交区。
 * 实现逻辑：
 * - 页面本身尽量保持轻量，只负责组织布局并消费 `useOrderEntryForm` 提供的状态与动作；
 * - 录入规则、草稿恢复和提交校验下沉到组合式函数，页面层只承接结构化展示与事件接线。
 * 维护说明：
 * - 若后续增加新卡片或步骤区，优先继续沿用“壳层编排 + composable 管流程”的拆分方式；
 * - 页面布局调整时要同步检查桌面端和移动端的录入连贯性，不要破坏现有键盘流。
 */
