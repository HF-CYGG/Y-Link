...
/**
 * 模块说明：src/components/common/page-container/PageContainer.vue
 * 文件职责：提供页面主体容器壳，统一页面标题、描述、内容宽度与上下留白，是管理端和客户端页面的基础承载结构。
 * 实现逻辑：
 * - 通过统一容器结构承接页面头部信息与主体内容插槽，减少业务页重复声明外层骨架；
 * - 根据设备态和参数控制最大宽度、内边距与标题区间距，保持全站页面节奏一致；
 * - 对少数需要窄版阅读宽度的页面提供可选收敛能力，同时不影响默认页面布局。
 */
