...
/**
 * 模块说明：src/layout/components/QuoteBanner.vue
 * 文件职责：提供管理端顶部栏中的语录横幅，负责拉取、展示与刷新一句话内容，补充布局层的人文信息区。
 * 实现逻辑：
 * - 组件挂载后尝试请求公开语录接口，并把结果裁剪成页面实际展示所需的最小字段；
 * - 当外部接口不可用时回退到本地语录池，保证顶部区域始终有稳定内容可展示；
 * - 刷新按钮只驱动当前组件内部状态更新，不影响布局层其余导航与账户交互。
 */
