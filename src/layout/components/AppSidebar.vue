 ...
 * 文件职责：渲染管理端布局侧边导航，统一承接菜单列表展示、当前路由高亮与移动端折叠状态适配。
 * 实现逻辑：
 * - 接收布局层派发的菜单元信息，集中输出侧边导航结构，避免页面各自拼装入口；
 * - 结合当前路由判断激活菜单项，保证导航高亮与真实访问页面保持一致；
 * - 通过 App Store 感知侧栏展开状态与设备模式，统一管理桌面常驻和移动端抽屉表现。
 */
