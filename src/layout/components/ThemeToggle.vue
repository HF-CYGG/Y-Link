 ...
 * 文件职责：提供管理端布局右上角的主题切换入口，负责展示当前明暗主题状态并触发全局主题切换动画。
 * 实现逻辑：
 * - 从 Theme Store 读取当前主题模式、切换中状态和对应图标，保证全局主题口径一致；
 * - 点击按钮时把触发位置交给 Store，供全局圆形扩散/收缩动画复用同一套起点；
 * - 组件本身只负责界面交互与状态映射，不直接持有主题持久化和根节点样式切换逻辑。
 */
