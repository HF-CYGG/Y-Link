 ...
 * 文件职责：维护前端应用级公共状态，统一承接设备模式判断、侧栏折叠态与全局加载等跨页面基础状态。
 * 实现逻辑：
 * - 通过 Pinia 作为全局单一真源，让布局层、共享组件与页面层共用同一份应用状态；
 * - 把设备断点结果抽象为 phone / tablet / desktop 三档，供响应式布局和共享壳层统一消费；
 * - 集中管理侧栏开合和全局请求反馈等高频 UI 状态，避免分散在各页面重复维护。
 */
