...
| Entrypoint | `/entrypoint.sh` |
| Command | 留空 |
| 环境变量 | 至少添加 `INIT_ADMIN_PASSWORD=你的私有强密码` |
| 特权模式 | 不开启 |
| 控制台交互 | 不需要开启 |
| 重启规则 | 首次排错可不重启，正式使用建议失败后重启 |

截图中只有 `PATH`、`NODE_VERSION`、`YARN_VERSION` 这类镜像自带变量还不够，必须额外添加 `INIT_ADMIN_PASSWORD`。
...
```bash
INIT_ADMIN_PASSWORD='请改成你自己的强密码' docker compose -f compose.cloud.yml up -d
```
