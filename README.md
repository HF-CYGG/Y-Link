...
同时请在反向代理中显式透传以下头，避免 Cookie 安全位与真实访客 IP 判定错误：

- `X-Forwarded-Proto`（必须，值应为 `https` 或 `http`）
- `X-Forwarded-For`
- `X-Real-IP`
...
| 日志 IP 总是 `10.255.0.1` | 容器只看到了 Docker/1Panel 的上一跳地址 | 确认 1Panel 反向代理传递 `X-Forwarded-For`、`X-Real-IP`、`X-Forwarded-Proto`，并使用新版镜像 |
...
当前版本默认仅对高频只读接口开启短缓存（显式配置）：

- `GET /api/o2o/mall/products`：20 秒
- `GET /api/o2o/mall/preorders`：12 秒
- `GET /api/o2o/mall/preorders/:id/summary`：12 秒

实现方式为“内存短缓存 + ETag 条件请求（304）”。认证接口与敏感接口继续 `Cache-Control: no-store`，不会进入缓存。
