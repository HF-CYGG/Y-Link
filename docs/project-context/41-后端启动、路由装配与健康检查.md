# Y-Link 后端启动、路由装配与健康检查

## 适用范围

- 适用于后端启动失败、健康检查异常、接口挂载顺序疑问和数据库模式判断。
- 适用于回答“后端启动时到底先做了什么、`/health` 为什么是判断真相的第一入口”。

## 最后核对来源

- `backend/src/index.ts`
- `backend/src/app.ts`
- `backend/src/config/data-source.ts`
- `backend/src/config/database-bootstrap.ts`
- `backend/src/config/database-runtime-override.ts`
- `backend/src/database/database-strategy.ts`
- `backend/src/database/transaction-coordinator.ts`
- `backend/src/runtime/rescue-app.ts`
- `backend/src/runtime/database-startup-preflight.ts`

## 真实入口

- 启动入口：`backend/src/index.ts`
- 应用装配入口：`backend/src/app.ts`
- 数据源入口：`backend/src/config/data-source.ts`
- 救援最小控制面：`backend/src/runtime/rescue-app.ts`

## 前端链路

- 管理端和本地联调脚本都依赖 `/health` 判断后端是否真正起来。
- 公开 `/health` 只承担存活探测和维护横幅，不向匿名访问者暴露数据库模式；数据库运行能力应从受保护的管理端接口读取。

## 后端链路

- 正常启动顺序大致为：
  1. 准备运行时数据库上下文
  2. 初始化数据源
  3. 初始化数据库策略与事务协调器：SQLite 校验 WAL/PRAGMA 并安装有界单写队列；MySQL 安装连接池等待保护
  4. 初始化数据库结构
  5. 迁移旧上传引用
  6. 确保默认管理员
  7. 确保默认系统配置
  8. 确保默认通知规则
  9. 启动 HTTP 服务并执行启动自检
- `app.ts` 先挂公共安全头，再挂上传静态资源，再挂公开路由，最后统一给受保护 `/api` 接口加管理端鉴权链路。
- 启动预检、业务运行时装配或数据库初始化失败时，`index.ts` 不会退回“半启动的普通应用”。它只启动不依赖业务数据库的救援控制面：`/health` 返回 HTTP `503` 与 `status=RESCUE`，普通业务路径统一返回维护响应。
- 救援控制面只挂载 `/api/database-rescue`；它不创建账号、会话、业务表、上传服务或后台 Worker。正常应用同样在维护写屏障之前挂载该路由，使受控恢复不依赖普通管理端会话。

## 关键状态/字段/快照

- `/health` 只返回 `status` 与脱敏维护态，不返回数据库拓扑、队列指标或运行时覆盖详情。
- `status=UP` 只表示正常业务运行时已就绪；`status=RESCUE` 表示业务运行时未装配，必须转入救援协议。两种响应都不应被用来推断数据库账号、连接串或协调器内部指标。
- 管理员接口 `GET /api/data-maintenance/database/performance` 返回数据库引擎能力与写协调器水位；运行时覆盖详情由 `GET /api/data-maintenance/db-migration/runtime-override` 提供。
- 真正生效的数据库模式可能来自环境变量，也可能来自运行时覆盖配置。
- SQLite 模式下所有受协调的写入共用有界单写队列；`DataSource.transaction` 与 `QueryRunner` 都按完整事务持有租约，`pendingWrites/rejectedWrites/timedOutWrites` 用于判断是否需要迁移 MySQL。
- 禁止通过 `DataSource.query` 或 `QueryRunner.query` 直接发送 `BEGIN/COMMIT/ROLLBACK/SAVEPOINT` 等事务控制 SQL：跨多次 `DataSource.query` 调用无法可靠识别事务所有者，应改用回调事务或 QueryRunner 生命周期 API。
- MySQL 模式在 HTTP 监听和后台 Worker 启动前校验关键表、增量字段及索引形状；存量库漏执行 `035/036` 时会给出目标脚本并阻止启动。

## 权限与安全边界

- 公开路由只包括少数认证/O2O/客户端反馈入口。
- 其余管理端 `/api` 请求统一先过 `requireAuth` 与 `requireAdminCsrf`。
- 启动日志不会输出敏感明文密码，只给脱敏摘要。
- `Y_LINK_TRUST_PROXY` 只接受逗号分隔的明确 IP 或 CIDR；空值代表不信任代理头。禁止使用 `true`、跳数或全网网段把任意 `X-Forwarded-*` 变成可信输入。

## 常见异常与排查顺序

1. 服务起不来：先看 `backend/src/index.ts` 的启动阶段日志卡在哪一环。
2. `/health` 可访问但业务不对：用管理员数据库性能接口和运行时覆盖接口确认当前进程实际连接的数据库模式。
3. 本地和线上表现不一致：对比 `app.ts` 的路由挂载顺序与部署方式。
4. SQLite 模式偶发锁问题：先看管理员数据库性能接口中的 `writeCoordinator` 队深、拒绝和超时，再看业务事务是否过大、是否绕开受支持事务 API、数据库是否位于本地持久化磁盘。
5. MySQL 升级后启动提示结构不完整：按错误列出的结构对象只执行对应增量脚本；`036` 必须先停止所有应用和通知 Worker，不能通过开启实体同步绕过存量库迁移。
6. 返回 `RESCUE`：保留运行时目录、任务、marker 与 override 文件，读取独立救援操作说明；不要删除控制文件后重启碰运气。

## 验证与回归关注点

- 改启动或装配逻辑后至少回归：后端可启动、`/health` 返回正常、管理端登录和客户端匿名商品接口正常。
- 改数据库模式或事务协调逻辑后回归：`transaction-coordinator:verify`、公开 `/health` 脱敏约束、管理员数据库性能接口、运行时覆盖状态与对应验证脚本。
