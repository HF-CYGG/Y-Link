...
- 手工开单不扣减产品或 SKU 库存，也不写库存流水；SKU `defaultPrice` 仅用于桌面表格和移动抽屉的单价预填，人工价格可覆盖且不会回写 SKU 默认价。单 current SKU 商品保留既有商品主价回写，多 current SKU 商品绝不回写商品主价，避免明细顺序改变主价。
...
- 出库链路至少回归：单/多 SKU 新建、历史无 SKU 详情、打印/PDF 规格、报表规格、人工价格覆盖、软删、恢复、永久删、合规标记编辑。
